...
Description: After entering the regular expression, repeatly match input string against regular expression
...
Description: The major feature for verbose mode is that NFA will not reset for every input string. Thus, each input starts from the current states instead of the initial states of NFA. In addition, verbose mode also prints out the transition table.
